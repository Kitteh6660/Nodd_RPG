...
Inventory.inventoryMenu = function() {
	...
	outputText("<br><b><u>Combat Details:</u></b><br>");
	outputText("<b>Attack:</b> " + player.weapon.attack + "<br>");
	outputText("<b>Accuracy:</b> " + player.weapon.accuracy + "<br>");
	outputText("<b>Defense:</b> " + player.armor.defense + "<br>");
	outputText("<b>Evasion:</b> " + player.armor.evasion + "<br>");


    if (player.keyItems.length > 0) outputText("<br><b><u>Key Items:</u></b><br>");
...
Inventory.unequipMenu = function() {
	...
	if (player.accessory.id != JewelryLib.NOTHING) {
	...
	if (player.upperGarment.id != "Nothing") {
	...
	if (player.lowerGarment.id != "Nothing") {
...
Inventory.unequipUpperwear = function() {
	clearOutput();
	var oldWear = lookupItem(player.upperGarment.id);
	player.upperGarment = Items.NOTHING;
	Inventory.takeItem(oldWear, Inventory.unequipMenu);
}
Inventory.unequipLowerwear = function() {
	clearOutput();
	var oldWear = lookupItem(player.lowerGarment.id);
...
Inventory.takeItem = function(itype, nextAction, overrideAbandon, source) {
	...
	if (temp >= 0) { //First slot go!
		...
		outputText("You place " + itype.longName + " into the " + inventorySlotName[temp] + " slot of your Ego Bracer's inventory, giving you " + player.itemSlots[temp].quantity + " of them.");
	...
	if (temp >= 0) {
		...
		outputText("You place " + itype.longName + " into the " + inventorySlotName[temp] + " slot of your Ego Bracer's inventory.");
        Inventory.itemGoNext();
		return;
	}
